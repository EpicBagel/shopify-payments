...
class PaymentBarrier(object):
	def process_request(self, request):
		...
		if request.user.is_authenticated() and not request.user.is_superuser and not request.user.charge_id and not request.path in [reverse(allowed_url) for allowed_url in allowed_urls]:
